    ...
    if platform.system() == 'Windows':
        ...
        with open(starter, 'w', encoding='utf-8', newline='\n') as f:
            f.write('#!/bin/bash\n'
                    ...
                    'echo "SCB Video-Render laeuft - Fenster offen lassen ..."\n'
                    'PY=""\n'
                    'for K in python3 python /opt/homebrew/bin/python3 /usr/local/bin/python3 /usr/bin/python3; do\n'
                    '  command -v "$K" >/dev/null 2>&1 && PY="$K" && break\n'
                    'done\n'
                    'if [ -z "$PY" ]; then\n'
                    '  echo "FEHLER: Python nicht gefunden - sag Claude: richte Python ein."\n'
